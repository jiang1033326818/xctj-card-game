...
// 测试删除重复admin账户API
function testDeleteDuplicateAdmins() {
  ...
  const options = {
    hostname: 'localhost',
    port: 3000,
    path: '/api/delete-duplicate-admins',
    method: 'POST',
    headers: {
  ...
  const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      try {
        ...
        if (result.success) {
          console.log('✅ 清理重复账户成功:', result.message);
        } else {
          console.log('❌ 清理重复账户失败:', result.error);
        }
      } catch (e) {
...
// 运行测试
console.log('测试清理重复admin账户功能...');
testDeleteDuplicateAdmins();
